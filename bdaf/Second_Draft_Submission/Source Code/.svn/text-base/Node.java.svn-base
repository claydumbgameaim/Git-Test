		...
		while(getRun())
		{
			if(sync == 1)
			{
				...
				if(this.getcurrentFile() >= PACKET_NUM - 1)
				{
					...
					transDelay = (double) sum - transDelay;
					System.out.println("DONE " + this.getNodeName() + ", time spend= " +sum + " with a throughput of " + throughput + " and has a delay of " + transDelay);
				}
			}
			else
			{
			...
			while(files.peek()!=null)
			{
				...
				if(done && files.peek() == null)
				{
					...
					throughput = this.getTotalData()/sum;
					transDelay = (double) sum - transDelay;
					System.out.println("DONE " + this.getNodeName() + ", time spend= " +sum + " with a throughput of " + throughput + " total delay: " + transDelay );
				}
			}
			}
		}
	...
	public void sentRTS() throws InterruptedException{
		...
		if(nodesInRange.size() != 0) //if there are nodes in range
		{
			...
			while(temp.receiveRTS(this,failed, packetSize, lastBackOffTime) == false)
			{
				...
				{
					System.out.println("fail :" + this.getNodeName() + " to " + temp.getNodeName() + ", idle status: " + temp.getIdle());
					failed = true;
