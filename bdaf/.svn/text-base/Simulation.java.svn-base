...
//		int numberOfPayloads = (int)Math.round(amountOfDataToSend/PAYLOAD_MAX_SIZE + 0.5);
		...
		double currentDataReceived = 0;
		Random rand = new Random();
		int randomPacketSize = 0;
		int numberOfPayloads = 0;

		// Calculates how many payloads are needed to send all the data sent by sender
		while(currentDataReceived < amountOfDataToSend)
		{
			// packet sizes vary to allow varying input traffic load
			randomPacketSize = rand.nextInt((int)PAYLOAD_MAX_SIZE);

			currentDataReceived+=randomPacketSize;

			numberOfPayloads++;
		}


		double totalDataReceived = numberOfPayloads * FRAME_OVERHEAD_SIZE;
